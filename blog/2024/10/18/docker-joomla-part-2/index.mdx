...
<TLDR>
  This guide details how to automate local Joomla development with Docker by
  combining `compose.yaml`, `.env`, and `makefile` configurations to deploy
  fully configured sites with a single command. It explains how to bypass the
  standard installation wizard by pre-defining environment variables and
  demonstrates how to manage multiple isolated projects with different Joomla
  and PHP versions simultaneously on one machine. The tutorial provides specific
  configuration examples to ensure data persistence and enable conflict-free
  operation across concurrent instances.
</TLDR>
...
<QuickJump
  links={[
    ...
    { label: "Before starting", to: "#before-starting" },
  ]}
/>
...
That's the whole deployment: Docker pulls Joomla, PHP, Apache and MySQL, wires them together, and
a minute later the site is reachable in the browser — no manual database setup, no editing PHP
files by hand.
...
- `compose.yaml`, `.env` and `makefile` together become a single point of configuration: one
  file to change the Joomla or PHP version, one for the port number and project name, and
  `make` targets so nobody has to remember the raw `docker compose` incantations.
- `depends_on` + `healthcheck` in `compose.yaml` make Joomla wait for the database to actually be
  ready before starting its own installation — no crash-on-boot race condition.
- Because the whole setup lives in three small files, the same recipe scales to several isolated
  projects (different Joomla/PHP/MySQL versions) on the same machine, as the *Advanced
  configuration* and *Starting three different projects* sections below show.

## Create the three configuration files

In contrast to article [Create your Joomla website using Docker - Part 1](/blog/docker-joomla), where I began by explaining a few concepts before moving on to discuss the subject, let's do the opposite here: let's start with the three files we are going to use, and then see how they work. Please don't try too hard to understand the contents of the files; we'll come back to that later.
...
In recent months, the name of the file to be used has changed from `docker-compose.yaml` to `compose.yaml`; the old name is still supported for now, but might as well use the new one. See [docs.docker.com](https://docs.docker.com/compose/intro/compose-application-model/#the-compose-file) if you want more info about this change.
...
Using a `.env` file is really useful so you no longer need to make changes to the `compose.yaml` file, which can stay standard across all your projects.
...
<AlertBox variant="caution">
If you don't know if you already have `GNU make`, just run `which make` in the console. If you see `make not found` then please run `sudo apt-get update && sudo apt-get install make` to complete the installation.
...
In your console, just run `make up`, and please wait one minute. The very first time, you'll get the console output shown at the top of this article.
...
As you can see, the port number is `8080`; the one we've specified in our `.env` file. Simply CTRL click on the URL if your terminal support this option (otherwise, manually start your preferred web browser and navigate to the specified URL) — you'll land on the installer screen shown at the top of this article.
...
<StepsCard
  ...
  steps={[
    ...
    "That with the `compose.yaml` file we tell Docker what to do to make Joomla and MySQL talk to each other, and",
...
Now, by going to `http://127.0.0.1:8080/administrator`, you'll no longer see the installation screens but, immediately, the login screen. The username to use is `joomla` and its password is `joomla@secured` (as you can see on the console).
...
<AlertBox variant="info" title="Project 2 will be a Joomla 4.0">
In a very few words: we'll install Joomla 4.0 and MySQL 5.7.37.

</AlertBox>

Please configure the `project_2/.env` with the following content. As you can see (highlighted lines), that file is the same as the latest `.env` we've used earlier but, just, we'll adjust the name of the project and the web port (so our site will be available on `http://127.0.0.1:8082`).
...
<AlertBox variant="info" title="Project 3 will be a Joomla 3.5">
...
Save the file and run `make up && make start` to start the third website. Go to `http://127.0.0.1:8083/administrator` and yes, we've our third site running on our machine.
...
As you can see, I actually have four projects: my current blog (running on Docker too) and the three Joomla projects.

If you look at the **CPU (%)** column, you can see that the projects are almost asleep. And this is normal: once the web page has been displayed, Joomla and MySQL don't require CPU anymore. The page is displayed so they are sleeping.

For the memory, I've allocated max. 20GB to WSL/Docker and right now, my four projects (including my blog) are eating 3.3 GB combined. I can thus start a lot more projects.

But do I need to be able to work on all three projects at the same time? Probably not. So why not stop, f.i., the first and the third project?
...
## Under the hood: Docker logs

*Optional — skip this section if everything is working and you don't need to debug.*
...
Three small files — `compose.yaml`, `.env`, `makefile` — and `make up` turns into your entire
Joomla deployment story: fresh installs, pre-configured sites that skip the installer wizard, and
as many isolated projects (different Joomla/PHP/MySQL versions) as your machine can carry, each
one started, stopped or torn down with a single command.
