...
Here's what `compose-viz` generates from a Joomla `compose.yaml`:
...
Let the magic happen and run `docker run --rm -it -u $(id -u):$(id -g) -v $(pwd):/in wst24365888/compose-viz compose.yaml` — you'll get the image shown above.
...
![More complex example](./images/more_complex.webp)
