...
<TLDR>
This article tests whether PHP code obfuscation (using the author's `php_obfuscator` script, which strips whitespace and renames identifiers to random strings) still protects proprietary code in the AI era. A hangman game generated by Gemini is obfuscated and still runs correctly, but Copilot is then able to fully de-obfuscate it back into readable, commented code — questioning whether obfuscation is still worthwhile.
</TLDR>
...
Now, we'll make this code unreadable by removing unneeded spaces and carriage returns, and replacing names with some random letters. For instance, replace the `playHangman` function name with a random string like `ppny`. And to make the code more unreadable, I'll choose only strings like `ppmx`, `ppnx`, `ppny` and so on.
...
The first time, [Copilot](https://copilot.microsoft.com/) has kept my minified function name. By asking him *Can you suggest another version?*, Copilot suggests this code:
...
Obfuscation aside, if you're after more conventional PHP code-quality tooling, check out <Link to="/blog/php-jakzal-phpqa">the Docker image bundling static analysis tools</Link>.
