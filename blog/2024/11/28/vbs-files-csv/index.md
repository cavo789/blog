...
Here's what this script produces: a CSV file listing every file in a folder, with dates, size, type, and owner.
...
Here's the script that generates it. Copy/paste the source below and save it to a file, let's say `c:\files2csv.vbs`. Start a DOS console, run `cd c:\` then `cscript files2csv.vbs`. When the job has been done, you'll get the listing in `files2csv.csv`.
