...
<TLDR>
This article covers the `sftp` command line: connecting with `sftp user@host`, automating authentication with `sshpass`, routing the connection through an HTTP proxy via `-o ProxyCommand='/usr/bin/nc --proxy-type http --proxy ...'`, using a custom port with `-P`, and scripting remote commands (like downloading all `.zip` files) with a here-document passed to `sftp`.
</TLDR>
...
In this article, we'll explore how to start an SFTP connection to a remote server, from the command line.
...
The sshpass Linux utility is used by the script to allow an SFTP connection without having to use the password in plain text. [Get more info](https://www.redhat.com/sysadmin/ssh-automation-sshpass)

</AlertBox>

Let's imagine your user is `christophe`, your password is `p@ssword` and the server IP is `1.2.3.4`, then you can connect like this: `SSHPASS="p@ssword" sshpass -e sftp christophe@1.2.3.4`.
...
As already seen, using `sshpass` we can bypass the authentication. We just need to make sure an `SSHPASS` variable exists just before calling the `sshpass` command.

Then, to be able to automate some commands on the SFTP server, we can use a *here-document* syntax.

The general look and feel will be this one:
