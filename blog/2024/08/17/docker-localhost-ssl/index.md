...
<TLDR>
This guide adds SSL to a Dockerized Apache site so `https://localhost` works without browser warnings. Instead of a self-signed OpenSSL certificate, it uses mkcert to generate a locally-trusted certificate covering both `localhost` and `127.0.0.1`. Bonus sections cover the same setup for nginx and PHP, plus how to import the mkcert root CA into Windows when your browser runs outside WSL2.
</TLDR>
...
Everything is now in place to enable http access. Still in the `/tmp/https_localhost` folder, now, just run `docker compose up -d --build` to build our custom Docker image and run a container i.e. start the website.
...
First, install [mkcert](https://github.com/FiloSottile/mkcert) if you haven't already:
...
Here we need to do a lot of stuff. We have to install (`apt-get install`) some Linux binaries to allow later use of `a2enmod` and the SSL protocol.
...
<AlertBox variant="danger" title="Be careful with file and folder names">
I spent many hours — too many — writing this article. It looked simple; there are several articles here and there that give the impression it would be easy, but it was not.

That's because the majority of examples I found used a PHP or Nginx image and not Apache. File and folder names are not the same, and we should carefully use the ones required by Apache. It was not explicit at all.
