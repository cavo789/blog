...
<TLDR>
This comprehensive guide covers essential best practices for designing clean, scalable, and semantically correct REST APIs. Learn the do's and don'ts of URI construction, including using plural nouns and avoiding verbs. The article provides a detailed breakdown of how to properly use HTTP verbs like `GET`, `POST`, `PUT`, `PATCH`, and `DELETE`, and what HTTP status codes to return in different scenarios. It also delves into structuring JSON responses for data, errors, and pagination, following established standards for robust API design.
</TLDR>
<!-- cspell:ignore Belgif -->

When <Link to="/blog/python-fastapi">developing APIs</Link> _from scratch_, you can do it in the mode _I get behind the PC and start programming_, or _I learn about the standards first and then program in compliance with <Link to="/blog/belgif-api-linter">these standards</Link>_.
...
_Path segments and query parameters should use lowerCamelCase notation to enhance readability and to separate compounds names._ ([reference](https://www.belgif.be/specification/rest/api-guide/#resource-uri))
...
It's **really recommended** to inform the server about what you expect for: `json`, `csv`, `plain-text`, `xml`, ... To do this, use the `Accept` header like in the example here above. This is safer because the server can change its default format from JSON to CSV f.i. and if you expect JSON, your code will be broken.
...
<AlertBox
  variant="info"
  title="You can translate `POST` as `CREATE` in `CRUD`"
/>
...
When calling `POST` you should provide all the fields required to create the resource. So, for a new employee, their firstname, lastname, birthdate, national register number, their hiring date, the department where they'll work and so on.
...
<AlertBox variant="caution" title="The RESTful API standards recommend using `POST` for actions.">
So, we shouldn't do `GET https://xxx/api/v1/employees/actions` because `actions` is not a resource but an action.
...
<AlertBox
  variant="info"
  title="You can translate `PUT` as `UPDATE` in `CRUD`, when you'll update every field"
>
  Using `PUT` implies you intend to update each field of the record. If you
  plan to update just a few of them, take a look at the `PATCH` verb.
</AlertBox>
...
Consider using `PUT` only when **you'll update every piece of information** of the resource (except calculated fields like f.i. `employee_id`). If you wish to update just a few ones (partial content), consider using `PATCH`.

<span style={{ color: "green" }}>
  `PUT` is idempotent since running the same update won't have other effects on
  the server. You can update once, five or one thousand times the firstname of
  the employee #999, the result will always be the same.
</span>
...
| `400` | `Bad request`   | The request is incorrect (f.i. you're trying to update a field `first_name` while its correct name is `firstname`). |
| `401` | `Unauthorized`  | The endpoint is secured, and the request didn't pass a valid access token.                                          |
| `403` | `Access denied` | You're not allowed to call this method                                                                              |
...
When getting `PUT -d '{"firstname":"Christophe"}' https://xxx/api/v1/employees/123456` the API developer can decide on their own to **CREATE** the resource 123456 if it didn't exist and if all required fields have been provided in the payload.

So, when the resource didn't exist, the developer can return a `404 - Not found` error or, if they decide to create the new resource, then the API could return `201 - Created` (just like a `POST`) ([reference](https://www.belgif.be/specification/rest/api-guide/#put)).
...
<AlertBox
  variant="info"
  title="You can translate `PATCH` as `UPDATE` in `CRUD`, when you DON'T update every field"
>
...
`PATCH` on an in-existing resource will return an error while, perhaps, `PUT` will create the resource. It's then safer to use `PATCH` and not `PUT` when updating partial content.

It's indeed impossible to create a new record with `PATCH` since the request just mentions little information, not all.
...
<AlertBox
  variant="info"
  title="You can translate `DELETE` as ... `DELETE` in `CRUD`"
/>
...
| Code  | Status          | Reason                                                                                                           |
| ----- | --------------- | ---------------------------------------------------------------------------------------------------------------- |
| `200` | `OK`            | The record was successfully deleted and a message like _The resource has been successfully deleted_ is returned. |
...
| `403` | `Access denied` | You're not allowed to call this method.                                                                          |
| `404` | `Not found`     | The requested resource didn't exist (f.i. you tried to access `/api/v1/employees/1` and that one didn't exist).  |
| `409` | `Conflict`      | The resource can't be deleted (f.i. you can't dismiss an employee while there are still payments to be made).    |
...
The error `403 Forbidden` has to be used too when a valid token is used but when the called resources require a specific scope (like `canManageCustomers`) and the used token doesn't have that one.
...
The request is indeed valid (the structure is correct) but the asked return type is not supported. For instance, by updating `application/xml` to `application/json` then the request can be processed.
...
In this example, the server didn't support `application/x-www-form-urlencoded` for that route but will accept `application/json`.
...
For example, the request has to define `-H "Accept: application/json"` in its header to let the server know which media type is needed and the server (the response) should provide the `Content-Type` too.
...
In order to make the API flexible, make sure the answer is a JSON object `{ ... }` and not an array `[ ... ]` because that last one won't allow adding extra properties in the future ([reference](https://www.belgif.be/specification/rest/api-guide/#rule-evo-object)).
...
- `data`: the document's _primary data_. This node has to be renamed to better describe the object, f.i. `employees` or `countries`, ...
...
The document's _primary data_ is a representation of the resource or collection of resources targeted by a request.
...
<AlertBox
  variant="info"
  title="Most of the time, it's the developer choice for an array"
>
  In fact, most of the time, an array is returned, as this allows the API to be
  modified in the future to return other results without generating BC (break
  changes) in the API.
</AlertBox>
...
`errors` has to be an array so we can return more than once and should contain at least one of these keys (_not exhaustive_):
