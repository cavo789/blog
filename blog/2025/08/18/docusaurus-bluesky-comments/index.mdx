...
Once wired up, every article gets a "Like, share or comment on Bluesky" button, the live likes/reposts count, and the list of comments straight from your Bluesky post — right below the article, no separate comment system to host.

<!-- TODO(author): capture a real screenshot of the rendered Bluesky block (button + likes/reposts counter + comments list) from the bottom of a published article here before the next edit — not reproducible in this session (requires a live, deployed Docusaurus build). Meanwhile, scroll to the bottom of this very article: the block rendered there is exactly what this component produces. -->
