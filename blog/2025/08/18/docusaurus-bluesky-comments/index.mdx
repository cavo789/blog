...
<QuickJump
  links={[
    { label: "What You'll Get", to: "#what-youll-get" },
...
In the previous article (see [Create our own Docusaurus React component and provide a "Share on Bluesky" button](/blog/docusaurus-bluesky-share)), we've created the `src/components/Bluesky/index.tsx` file. Please edit that file and use this new content:

## 1/6 The layout of our Bluesky generic component
...
## 2/6 The shared data-fetching hook

Both `index.tsx` (for the headline copy) and `likes.tsx` (for the counts and the facepile) need the same Bluesky data, so instead of fetching it twice, we centralize everything in one custom hook: `useBlueskyEngagement.ts`. It resolves the `blueskyRecordKey` (from your front matter or, if you didn't set one, by matching the article's URL against your own Bluesky post history) and fetches likes, reposts and comments in a single pass.
...
## 3/6 Our "Like, share or comment" button
...
## 4/6 Our component for getting the number of likes and reposts
...
Here is the full set of files this article touched — the update to `index.tsx`, and the four new files (`useBlueskyEngagement.ts`, `post.tsx`, `likes.tsx`, `comments.tsx`) plus the refreshed CSS. Use the button below to copy them all at once, or download them as a ready-to-extract ZIP:

<ProjectSetup folderName="Bluesky engagement" createFolder={false}>
  <Guideline>
    These files replace the ones created in the [first article](/blog/docusaurus-bluesky-share).
    Don't forget to also add the `customFields.bluesky` block to your `docusaurus.config.js` (see
    above), then restart your Docusaurus dev server.
  </Guideline>
