...
Both `index.js` (for the headline copy) and `likes.js` (for the counts and the facepile) need the same Bluesky data, so instead of fetching it twice, we centralize everything in one custom hook: `useBlueskyEngagement.js`. It resolves the `blueskyRecordKey` (from your front matter or, if you didn't set one, by matching the article's URL against your own Bluesky post history) and fetches likes, reposts and comments in a single pass.

<Snippet
  filename="src/components/Bluesky/useBlueskyEngagement.js"
  source="src/components/Bluesky/useBlueskyEngagement.js"
/>
...
Here is the full set of files this article touched — the update to `index.js`, and the four new files (`useBlueskyEngagement.js`, `post.js`, `likes.js`, `comments.js`) plus the refreshed CSS. Use the button below to copy them all at once, or download them as a ready-to-extract ZIP:
