...
Do a `Ctrl + Click` on the URL (`http://localhost:`<Var name="port">4242</Var>), and it will open your browser with the rendered Quarto document. You should see the Mermaid diagrams generated by our Python scripts.
...
Most people think Doc-as-Code is just "saving Markdown in Git." It's not. It's about **runtime execution**.

- A Quarto document isn't static Markdown — a `{python}` code block embedded in the document actually **executes during render**, not before it.
- `output: asis` tells Quarto to treat whatever that Python function prints as raw Markdown (or Mermaid) instead of literal text — so a function that returns a diagram becomes a rendered diagram, not a code listing.
- Because the diagram is generated from the *live* `compose.yaml` and `Dockerfile`, changing the infrastructure and re-rendering updates the diagram automatically — no manual redraw, ever.

## Installation — The "Wow" Lab: Build Your First Doc-Engine in 5 Minutes
...
<Vars port="4242" labels={{ port: "Host port" }} />
