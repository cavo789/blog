...
Great no? But where does this diagram come from? It’s generated by the `render_project_arch()` function in our `project_viz.py` script. That script looks at our `compose.yaml` and our `Dockerfile` to understand the structure of our project and generates a Mermaid diagram based on that.

Let’s look at the output of our `file_stats.py` script. It analyzes the files in our project and generates a report that is rendered directly in our Quarto document.
