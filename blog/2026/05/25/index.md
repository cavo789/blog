...
authors: [christophe]
image: /img/v2/fproj_with_fzf.webp
mainTag: fzf
ai_assisted: true
tags: [fzf, zsh]
date: 2026-05-25
---
![Modular ZSH Workflow](/img/v2/fproj_with_fzf.webp)
...
In this article, I’ll show you why you should move your logic into standalone files within `~/.zsh/fns` and how ZSH's `autoload` mechanism can make your terminal feel snappier than ever. To top it off, I’ll share `fproj`, a function that will change how you navigate your workspace.
...
## 💎 The Super Function: `fproj` (Universal Project Navigator)
...
The `fproj` function uses `fd` (or `find`) and `fzf` to let you search all your git repositories and:
...
This function contains logic to handle directory depth and editor launching. In your `.zshrc`, it's clutter. In `~/.zsh/fns/fproj`, it's a dedicated tool.
...
### 3. Create the `fproj` function file

Create a file at `~/.zsh/fns/fproj` (no extension!) and paste the following code.

<ProjectSetup folderName="~/.zsh/fns" createFolder={true} >
  <Guideline>
    Ensure the filename is exactly "fproj". Do not add .sh or .zsh,
  </Guideline>
  <Snippet filename="fproj" source="./files/fproj.zsh" defaultOpen={true}/>
</ProjectSetup>
...
Create a new terminal or run `source ~/.zshrc` (because you've updated `.zshrc`) and simply run `fproj` to start the function. So cool no?

<AlertBox variant="tip" title="Reload">

If you've already used the alias and, for some reason, you've updated the code, the command to run is: `unfunction fproj && autoload -Uz fproj && fproj`.
...
With `fproj` living in your new modular setup, you can now teleport across your workspace in seconds. Next time you write a useful snippet, don't paste it in your `.zshrc`. Give it its own home in `~/.zsh/fns/`.
