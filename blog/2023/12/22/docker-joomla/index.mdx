...
<UpdateAt
  ...
  updates={[
    { date: "2024-03-23", content: "adding health condition in yml and update Joomla tag to 5.1-php8.2-apache" },
...
<AlertBox variant="info" title="The Docker Joomla image is built on PHP and Apache">
...
During writing this article, Joomla *latest* correspond to Joomla version 5.3.3. So, what about to force to use Joomla 5.0. By surfing on the [tags](https://hub.docker.com/_/joomla/tags) page, you can retrieve in the list of tags this one: *joomla:5.3.3-php8.2-apache*. So just replace `image: joomla` with `image: joomla:5.3.3-php8.2-apache` in `compose.yaml` and it's done. You're forcing a version. **Note: make sure to use a tag ending by `-apache`.**
...
<Terminal>
$ docker compose kill && docker compose up --detach
</Terminal>
