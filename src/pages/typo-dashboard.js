...
function ReportCard({ report, slug, siteUrl, onResolve, onDelete }) {
  ...
  return (
    <div className={`${styles.reportCard} ${report.resolved ? styles.resolved : ""}`}>
      ...
      {report.context && <p className={styles.reportContext}>…{report.context}…</p>}
      {report.comment && (
      ...
      <div className={styles.reportActions}>
        ...
        <button
          ...
          onClick={() => {
            if (window.confirm("Delete this report permanently?")) onDelete(report.id);
          }}
        >
          Delete
        </button>
...
export default function TypoDashboard() {
  ...
  useEffect(() => {
    const initial =
      window.location.hash.slice(1) || localStorage.getItem(STORAGE_KEY) || "";
