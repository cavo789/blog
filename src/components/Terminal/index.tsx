...
type HeadingPrefixMap = Map<unknown, string>;

function getCopyText(
  children: ReactNode,
  headingPrefixMap: HeadingPrefixMap,
  separator = "\n",
): string {
  let text = "";
  if (Array.isArray(children)) {
    ...
    const elProps = el.props as { children?: ReactNode };
    const prefix = headingPrefixMap?.get(el.type) ?? "";
    if (elProps.children) {
      const childSeparator = el.type === Fragment ? "" : "\n";
      text += prefix + getCopyText(elProps.children, headingPrefixMap, childSeparator);
    }
  }
  return text;
}
...
function autoSpeed(lineCount: number) {
...
export default function Terminal({
  ...
  const headingPrefixMap = useMemo<HeadingPrefixMap>(() => {
