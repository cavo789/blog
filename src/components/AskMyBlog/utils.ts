 ...
 */
export function buildSearchIndex(questions: QuestionEntry[]): SearchIndex {
...
function idf(docFrequency: Map<string, number>, docCount: number, term: string) {
...
function expandTerm(docFrequency: Map<string, number>, term: string): string[] {
...
export function search(index: SearchIndex, query: string, limit = 8): QuestionEntry[] {
  ...
  const scored: { entry: QuestionEntry; score: number }[] = [];
