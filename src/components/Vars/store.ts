...
export function getServerSnapshot(): Overrides {
  return EMPTY;
}

export type VarResolver = (name: string, defaultValue: string) => string;
 ...
 */
export function useVarResolver(): VarResolver {
  const overrides = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
  return useCallback(
    (name: string, defaultValue: string) => overrides[name] ?? defaultValue,
    [overrides],
  );
}
