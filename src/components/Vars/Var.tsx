 ...
 * resolve internally (see substitute.ts) — for plain prose instead. MDX text
 ...
 * whole command in one code span instead of three, see Code.tsx — pass
 * `bare`: `Code` sets it automatically on every `<Var>` it finds among its
 * children, never set it by hand outside that context.
 */
export default function Var({ name, children, bare = false }: Props): JSX.Element {
