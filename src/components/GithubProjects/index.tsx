...
interface Props {
  /** GitHub username to fetch repositories for (required) */
  username: string;
}
...
// Sort repositories
const sortRepos = (repos: Repo[], isArchived: boolean): Repo[] => {
  if (isArchived) {
...
export default function GithubProjects({ username }: Props): JSX.Element {
  ...
  const loadCachedRepos = useCallback((): Repo[] | null => {
  ...
  const renderRepoCard = (repo: Repo, isArchived = false) => {
